...
def loadRAW(files, Phi_min=-45, Phi_max=45, Phi_step=1, Pitch=100, substratethickness=700 * 10 ** -6,
            substrateattenuation=200 * 10 ** -6):
    qx, qz, I = [], [], []
...
def fitting_cmaes(qx, qz, I, H=10, LL=20, Beta=70, Num_trap=5, DW=0.11, I0=3, Bkg=1):
    ...
    best_corr, best_fitness = fitting.cmaes(data=I, qx=qx, qz=qz, initial_guess=np.asarray(initiale_value), sigma=200,
                                            ngen=1, popsize=100, mu=10, N=len(initiale_value), restarts=0,
                                            verbose=False, tolhistfun=5e-5, ftarget=None)
