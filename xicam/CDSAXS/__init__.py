import os, sys, time
from xicam.CDSAXS import simulation, fitting, cdsaxs, data_treatment, custom_widget
...
class plugin(base.plugin):
    ...
    def update_right_widget(self):
        guiinvoker.invoke_in_main_thread(self.datatreatmentparam.param('Fit output', 'num trap fit').setValue,
                                         self.Num_trap)
    ...
    def datatreatment(self):
        Phi_min, Phi_max, Phi_step, Pitch = self.datatreatmentparam['Experimental input', 'Start angle'], \
                                            self.datatreatmentparam['Experimental input', 'End angle'], \
                                            self.datatreatmentparam[
                                                'Experimental input', 'Angle step'], self.datatreatmentparam[
                                                'Experimental input', 'Line pitch']
        substratethickness, substrateattenuation = self.materialparam['Material', 'Substrate thickness'], \
                                                   self.materialparam['Material', 'Substrate attenuation']
        fitrunnable = threads.RunnableMethod(data_treatment.loadRAW, method_args=(
        self.files, Phi_min, Phi_max, Phi_step, Pitch, substratethickness, substrateattenuation),
                                             callback_slot=self.diplay_experimentaldata)
        threads.add_to_queue(fitrunnable)
    ...
    def fit(self):
        ...
        self.H, self.w0, self.Beta1, self.Num_trap = self.datatreatmentparam['Initial line Profile', 'Heigth'], \
                                                     self.datatreatmentparam['Initial line Profile', 'Linewidth'], \
                                                     self.datatreatmentparam['Initial line Profile', 'Sidewall angle'], \
                                                     self.datatreatmentparam['Initial line Profile', 'Number trapezoid']

        self.best_param = [self.DW, self.I0, self.Bkg, self.H, self.w0] + [int(self.Beta1) for i in
                                                                           range(0, self.Num_trap, 1)]
        self.Ifit = data_treatment.SL_model1(self.qx, self.qz, self.best_param)
        self.diplay_fitteddata(self.Ifit, self.best_param)

        fitrunnable1 = threads.RunnableMethod(data_treatment.fitting_cmaes, method_args=(
        self.qx, self.qz, self.I, self.H, self.w0, self.Beta1, self.Num_trap), callback_slot=self.diplay_fitteddata)
    ...
    def diplay_fitteddata(self, Ifit, best_param, fval=None):
        if fval is None:
            ...
            self.Ifit = Ifit
            self.best_param = [best_param[0], best_param[1], best_param[2], best_param[3], best_param[4]] + [
                best_param[i] for i in range(5, len(best_param), 1)]
            self.fval = fval
